...
function SocialLink({ href, label, children }: { href: string; label: string; children: React.ReactNode }) {
  return (
    <a
      ...
      aria-label={label}
      className="flex h-9 w-9 items-center justify-center rounded-full border border-alabaster/20 text-alabaster/50 transition-all duration-200 hover:border-gold hover:text-gold hover:bg-gold/10"
    >
      {children}
    </a>
  );
